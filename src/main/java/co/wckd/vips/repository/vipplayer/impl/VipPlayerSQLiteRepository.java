package co.wckd.vips.repository.vipplayer.impl;
...
public class VipPlayerSQLiteRepository implements VipPlayerRepository {
    ...
    public void insert(UUID uuid, VipPlayer vipPlayer) {
        try (Connection connection = databaseConnection.getConnection(false);
            ...
            for (Vip vip : vipPlayer.getVips().values()) {
                ...
                statement.setBoolean(4, (vipPlayer.getActive() != null && vipPlayer.getActive().equals(vip)));
                statement.addBatch();
            }
    ...
    @Override
    public void deleteSingle(UUID uuid, Vip vip) {
        try (Connection connection = databaseConnection.getConnection(true);
